...
void meet(char *parent_name, char *child_info, bool print){
    ...
    if(strcmp(parent_name, friend_name) == 0){
        ...
        //fcntl(parent_to_child[1], F_SETFD, fcntl(parent_to_child[1], F_GETFD) | FD_CLOEXEC);
        //if(parent_to_child[0] != 3 && parent_to_child[0] != 4) fcntl(parent_to_child[0], F_SETFD, fcntl(parent_to_child[1], F_GETFD) | FD_CLOEXEC);
        //if(child_to_parent[1] != 3 && child_to_parent[1] != 4) fcntl(child_to_parent[1], F_SETFD, fcntl(child_to_parent[0], F_GETFD) | FD_CLOEXEC);
        if(parent_to_child[1] != 3 && parent_to_child[1] != 4) fcntl(parent_to_child[1], F_SETFD, fcntl(parent_to_child[1], F_GETFD) | FD_CLOEXEC);
        if(child_to_parent[0] != 3 && child_to_parent[0] != 4) fcntl(child_to_parent[0], F_SETFD, fcntl(child_to_parent[0], F_GETFD) | FD_CLOEXEC);
        //fcntl(child_to_parent[0], F_SETFD, fcntl(child_to_parent[0], F_GETFD) | FD_CLOEXEC);
        pid_t pid = fork();
        if(pid == 0){
            ...
            if(parent_to_child[0] != PARENT_READ_FD && parent_to_child[0] != PARENT_WRITE_FD) close(parent_to_child[0]);
            if(child_to_parent[1] != PARENT_WRITE_FD && child_to_parent[1] != PARENT_READ_FD) close(child_to_parent[1]);
        ...
        while(cur != NULL){
            ...
            if(print) snprintf(command, sizeof(command), "Meet %s %s true", parent_name, child_info);
            else snprintf(command, sizeof(command), "Meet %s %s false", parent_name, child_info);
            char buf[20];
...
bool checknode(char *check_friend_name, int task){
    if(strncmp(check_friend_name, friend_name, max(strlen(check_friend_name), strlen(friend_name))) == 0){
        ...
        while(cur != NULL){
            ...
            if(task == 1 && strcmp(cur->name, check_friend_name) == 0){
                snprintf(command, sizeof(command), "Collect");
            }
            else if(task == 1){
                snprintf(command, sizeof(command), "Check %s", check_friend_name);
            }
            else if(task == 2 && strcmp(cur->name, check_friend_name) == 0){
                memset(command, 0, sizeof(command));
                snprintf(command, sizeof(command), "graduate");
            }
            else if(task == 2){
            ...
            if (send_command_to_child(cur, command, buf)) {
                if (task == 1) {
                    ...
                    if(strcmp(cur->name, check_friend_name) == 0 && strncmp(buf, "Exited", 6) == 0){
                        if (prev == NULL){
                            head = cur->next;
                        } 
                        else{
...
void be_adopted(int parent_value){
    ...
    while (cur != NULL) {
        ...
        snprintf(meet_command, sizeof(meet_command), "Meet %s %s false\n", friend_name, cur->info);
   ...
   // fprintf(stderr, "debug365: %s\n", friend_info);
    ...
    if(is_Not_Tako()){
        ...
        while(read(PARENT_READ_FD, command, sizeof(command) - 1) > 0){
            if(strncmp(command, "Meet", 4) == 0){
                char parent_name[MAX_FRIEND_NAME_LEN] = {0}, child_info[MAX_FRIEND_INFO_LEN] = {0}, tf[7];
                sscanf(command + 5, "%s %s %s", parent_name, child_info, tf);
                if(strcmp(tf, "true") == 0) meet(parent_name, child_info, true);
                else meet(parent_name, child_info, false);
