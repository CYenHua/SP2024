...
void spawn_thread(int argc, char *argv[]) {
    char *padding[128]; //prevent the stack from being corrupted by the setjmp() and longjmp() functions
    (void)
    ...
    while (argv_idx < argc) {
        ...
        int *thread_arg; //定義一個指標 thread_arg，用來保存該線程的參數。
        ...
        if (routine_type == 1) {
            ...
            thread_arg = calloc(1, sizeof(int)); //為線程分配一個整數類型的參數空間，這個參數將存儲 Fibonacci 函數的輸入。
            thread_arg[0] = atoi(argv[argv_idx++]); //將 argv 中的數字參數轉換為整數，並將其傳遞給線程。
        ...
        //fprintf(stderr, "debug:109\n");
        thread_create(routine, thread_id++, thread_arg); //創建一個新的線程，並將對應的 routine 和 thread_arg 傳遞給它。
    }
}
